...
```{r}
values_to_removeGrype <- c("G_X.XX.X")
...
through_time_alpha_trivy <- function(d) {
  ...
  #ggsave(paste0("YOUR PATH","/reu-2023-cloud-tools/03_Analysis/04_product/imageCountChange_Trivy.pdf"), plot = p, width =10, height = 7)
}
...
through_time_alpha_merged <- function(d) {
  ...
  #ggsave(paste0("YOUR PATH","/reu-2023-cloud-tools/03_Analysis/04_product/imageCountChange_Difference.pdf"), plot = p, width =10, height = 7)
}
...
image_change_status_grype <- imageCounts_grype %>%
  group_by(imageName) %>%
  summarize(changeStatus = vulnerability_change_status(count)[[1]], diff = vulnerability_change_status(count)[[2]]) %>% summarize(changeStatus = vulnerability_change_status(count)[[1]], diff = vulnerability_change_status(count)[[2]]) %>%
  arrange(desc(diff))
...
ggplot(change_counts_grype, aes(x = "", y = count, fill = changeStatus)) +
  ...
  ggtitle(
    "Counts of Docker Images with Changing and Non-changing Vulnerability Counts within Grype"
  )
...
# Take the top 30 columns with the highest diff values
image_change_status_merged_filtered <-
  image_change_status_merged %>%
  slice_head(n = 15) %>%
  mutate(imageName = factor(imageName, levels = rev(unique(imageName))))

ggplot(image_change_status_grype_filtered, aes(x = imageName, y = diff)) +
  geom_point(color = "steelblue", size = 3) + olor = "steelblue", size = 3) +
...
ggplot(image_change_status_merged_filtered,
       aes(x = imageName, y = diff)) +
