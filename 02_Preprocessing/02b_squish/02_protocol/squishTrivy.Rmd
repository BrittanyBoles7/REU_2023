...
# Going through each Trivy version in sequence
for (trivy_version in trivy_list) {
  print(trivy_version)
  csv_file_path <- paste0(trivy_path, trivy_version)
  file_list <- list.files(csv_file_path)
  data_list <- list()
  
  # Extract the image name from the file name
  image_name <- str_remove(file_list, "Trivy.csv")
  ...
  for (i in seq_along(file_list)) {
    ...
    data <-
      read_csv(here(csv_file_path, file), show_col_types = FALSE)
